...
function checkActiveHours(start, end, currentHour) {
    if (start === end)
        return true;
    if (start < end) {
...
export class SuiVaultClient {
    ...
    buildCreateVault(params) {
        ...
        tx.moveCall({
            ...
            arguments: [
                ...
                tx.pure.vector("address", params.policy.allowedRecipients.map((r) => r)),
     ...
     */
    buildSpend(vaultId, keyId, amount, recipient, walrusBlobId = "") {
        const tx = new Transaction();
        tx.moveCall({
     ...
     */
    buildFreezeVault(vaultId, capId) {
        const tx = new Transaction();
        tx.moveCall({
     ...
     */
    buildUnfreezeVault(vaultId, capId) {
        const tx = new Transaction();
        tx.moveCall({
     ...
     */
    buildDeposit(vaultId, capId, coinObjectId) {
        const tx = new Transaction();
        tx.moveCall({
     ...
     */
    buildWithdraw(vaultId, capId, amount) {
        const tx = new Transaction();
        tx.moveCall({
     ...
     */
    buildRevokeKey(vaultId, capId, keyId) {
        const tx = new Transaction();
        tx.moveCall({
     ...
     */
    buildUpdatePolicy(vaultId, capId, policy) {
        const tx = new Transaction();
        tx.moveCall({
            ...
            arguments: [
                ...
                tx.pure.vector("address", policy.allowedRecipients.map((r) => r)),
     ...
     */
    buildSpendForDeepBook(vaultId, keyId, amount, pool, price, walrusBlobId = "") {
        const tx = new Transaction();
        tx.moveCall({
     ...
     */
    buildLogBlockedSpend(vaultId, keyId, amount, target, reason, walrusBlobId) {
        const tx = new Transaction();
        tx.moveCall({
     ...
     */
    buildIssueNewKey(vaultId, capId, agentAddress, agentName, keyDurationMs) {
        const tx = new Transaction();
        tx.moveCall({
     ...
     */
    async getVaultsByOwner(ownerAddress) {
        try {
            const caps = [];
     ...
     */
    async getAgentKeys(agentAddress) {
        try {
            const keys = [];
            let hasNextPage = true;
            let nextCursor = null;
     ...
     */
    async getSpendingHistory(vaultId, limit) {
        try {
     ...
     */
    async checkSpendWouldSucceed(vaultId, amount, recipient) {
        ...
        if (!checkActiveHours(vault.policy.activeHoursStart, vault.policy.activeHoursEnd, currentHour)) {
    ...
    async getVaultStats(vaultId) {
        ...
        const utilizationPercent = maxPerDay > 0n ? Number((todaySpent * 100n) / maxPerDay) : 0;
        let keyExpiryMs = null;
        if (vault.agentKeyId) {
 ...
 */
export function parseVaultError(errorCode) {
    const errors = {
