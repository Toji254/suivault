...
function getId(raw) {
    ...
    if (fields?.id?.id)
        return fields.id.id;
    if (fields?.id)
        return typeof fields.id === "string" ? fields.id : fields.id.id;
    return "";
}
/**
 ...
 */
function parseOptionId(optionField) {
    if (!optionField)
        return null;
 ...
 */
export function parseVaultOwnerCap(raw) {
    const fields = getFields(raw);
    const id = getId(raw);
    if (!fields) {
 ...
 */
export function parseAuditEntry(raw) {
    const fields = getFields(raw);
    const id = getId(raw);
    if (!fields) {
