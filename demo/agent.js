...
async function main() {
    ...
    if (!CONFIG.vaultId || !CONFIG.agentPrivateKey || !CONFIG.ownerPrivateKey) {
        console.error(chalk.red("❌ Error: Config parameters missing. Please run the setup script first:\n" +
            "   npm run setup"));
        process.exit(1);
    }
    ...
    console.log(chalk.cyan("╠══════════════════════════════════════════════════════════╣"));
    console.log(chalk.cyan(`║  Vault:  ${vaultId.substring(0, 10)}...${vaultId.substring(vaultId.length - 8)}               ║`));
    console.log(chalk.cyan(`║  Key:    ${keyId.substring(0, 10)}...${keyId.substring(keyId.length - 8)}               ║`));
    ...
    async function getVaultDetails() {
        ...
        if (!v)
            return { balanceStr: "0.0000 SUI", dailyStr: "0.0/0.3 SUI" };
        return {
    ...
    try {
        const amount = 20000000n; // 0.02 SUI
    ...
    try {
        const amount = 250000000n; // 0.25 SUI
        const tx = vaultClient.buildSpend(vaultId, keyId, amount, whitelistAddr);
    ...
    try {
        const amount = 10000000n; // 0.01 SUI
    ...
    console.log(`   📋 Recipient: ${whitelistAddr.substring(0, 10)}...  ${chalk.green("✅ whitelisted")}`);
    try {
        const amount = 50000000n; // 0.05 SUI
        const tx = vaultClient.buildSpend(vaultId, keyId, amount, whitelistAddr);
        const start = Date.now();
        const result = await suiClient.signAndExecuteTransaction({
            transaction: tx,
    ...
    try {
        const amount = 30000000n; // 0.03 SUI
        const tx = vaultClient.buildSpend(vaultId, keyId, amount, whitelistAddr);
        const start = Date.now();
        const result = await suiClient.signAndExecuteTransaction({
            transaction: tx,
