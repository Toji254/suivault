...
function initAmberCascades(canvas: HTMLCanvasElement) {
  ...
  function render(timestamp: number) {
    ...
    ctx.fillStyle = '#02040A'; // Deep custom Sui background color
    ...
    for (const col of columns) {
      ...
      for (let j = 0; j < col.length; j++) {
        ...
        if (j === 0) { r = 220; g = 245; b = 255; }
        else if (j < 3) { r = 140; g = 210; b = 240; }
        else { r = 30; g = 106; b = 255; } // Sui theme primary blue tint

        ctx.fillStyle = `rgba(${r},${g},${b},${brightness})`;
        if (j === 0) {
          ctx.shadowColor = 'rgba(30, 106, 255, 0.6)';
          ctx.shadowBlur = 8;
        }
    ...
    waterGrad.addColorStop(0, 'rgba(6, 16, 37, 0.6)');
    waterGrad.addColorStop(1, 'rgba(2, 4, 10, 0.95)');
    ...
    ctx.strokeStyle = 'rgba(30, 106, 255, 0.25)';
    ctx.lineWidth = 1.5;
    ctx.stroke();
